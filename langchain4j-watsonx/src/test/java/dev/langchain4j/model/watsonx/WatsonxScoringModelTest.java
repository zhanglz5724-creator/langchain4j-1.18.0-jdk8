...
public class WatsonxScoringModelTest {
    ...
    void testWatsonxRerankModelBuilder() throws Exception {
        ...
        List<RerankResult> rerankResults = Arrays.asList(
        ...
        try (MockedStatic<HttpClientProvider> httpClientProvider = mockStatic(HttpClientProvider.class)) {
            ...
            scoringModel.scoreAll(Arrays.asList(TextSegment.from("test1"), TextSegment.from("test2")), "query");
    ...
    void testScoreAll() {
        ...
        when(mockRerankService.rerank("query", Arrays.asList("test1", "test2"), null)).thenReturn(rerankResponse);
        ...
        withRerankServiceMock(() -> {
            ...
            var result = scoringModel.scoreAll(Arrays.asList(TextSegment.from("test1"), TextSegment.from("test2")), "query");
    ...
    void testScoreAllWithParameters() {
        ...
        withRerankServiceMock(() -> {
            ...
            when(mockRerankService.rerank("query", Arrays.asList("test1", "test2"), parameters))
                    .thenReturn(rerankResponse);

            var result = scoringModel.scoreAll(
                    Arrays.asList(TextSegment.from("test1"), TextSegment.from("test2")), "query", parameters);
            assertEquals(2, result.content().size());
            assertEquals(0.0, result.content().get(0));
            assertEquals(0.1, result.content().get(1));

            assertEquals(0, scoringModel.scoreAll(null, "query").content().size());
            assertEquals(0, scoringModel.scoreAll(Arrays.asList(), "query").content().size());
            assertEquals(
                    0,
                    scoringModel
                            .scoreAll(Arrays.asList(TextSegment.from("test1")), null)
                            .content()
                            .size());
            assertEquals(
                    0,
                    scoringModel
                            .scoreAll(Arrays.asList(TextSegment.from("test1")), "")
                            .content()
                            .size());
        });
    }
