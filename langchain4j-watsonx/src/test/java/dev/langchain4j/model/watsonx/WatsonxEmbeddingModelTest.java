...
public class WatsonxEmbeddingModelTest {
    ...
    void testWatsonxEmbeddingModelBuilder() throws Exception {
        ...
        List<EmbeddingResponse.Result> results = Arrays.asList(new EmbeddingResponse.Result(Arrays.asList(0f, 1f), "test1"));
    ...
    void testEmbeddingAll() {
        ...
        List<EmbeddingResponse.Result> results = Arrays.asList(
                new EmbeddingResponse.Result(Arrays.asList(0f, 1f), "test1"),
                new EmbeddingResponse.Result(Arrays.asList(0f, 1f), "test2"));

        when(mockEmbeddingService.embedding(Arrays.asList("test1", "test2"), null))
        ...
        withEmbeddingServiceMock(() -> {
            ...
            var response = embeddingModel.embedAll(Arrays.asList(TextSegment.from("test1"), TextSegment.from("test2")));
            assertEquals(2, response.content().size());
            assertEquals(Embedding.from(Arrays.asList(0f, 1f)), response.content().get(0));
            assertEquals(Embedding.from(Arrays.asList(0f, 1f)), response.content().get(1));
        });
    }
    ...
    void testEmbeddingAllWithParameters() {
        ...
        withEmbeddingServiceMock(() -> {
            ...
            when(mockEmbeddingService.embedding(Arrays.asList("test1", "test2"), parameters))
                    .thenReturn(new EmbeddingResponse("modelId", "createdAt", results, 10));

            var response =
                    embeddingModel.embedAll(Arrays.asList(TextSegment.from("test1"), TextSegment.from("test2")), parameters);
            assertEquals(2, response.content().size());
            assertEquals(Embedding.from(Arrays.asList(0f, 1f)), response.content().get(0));
            assertEquals(Embedding.from(Arrays.asList(0f, 1f)), response.content().get(1));

            assertEquals(0, embeddingModel.embedAll(null).content().size());
            assertEquals(0, embeddingModel.embedAll(Arrays.asList()).content().size());
        });
    }
